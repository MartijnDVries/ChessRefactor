        ...
        if abs(int(piece.square) - new_square) == 7\
            ...
            if not self.positionHandler.hasPiece(new_square):
                ...
                if self.move.is_legal(ep_move, piece.name):
                    self.positionHandler.setEnpassantMove(ep_square, new_square, piece.square)
                    self.updateEvent(piece, new_square)
                    return True
        ...
        if piece.color == "WHITE":
            if new_square in white_kingcastle_squares:
                if self.move.is_legal('O-O', "KING"):
                    ...
                    rook.update(self.positionHandler.getPositionFromSquare(F1),  F1)
                    piece.update(self.positionHandler.getPositionFromSquare(G1), G1)
                    self.Game.check_game_outcome()
                    return True
            if new_square in white_queencastle_squares:
                if self.move.is_legal('O-O-O', "KING"):
                    ...
                    rook.update(self.positionHandler.getPositionFromSquare(D1), D1)
                    piece.update(self.positionHandler.getPositionFromSquare(C1), C1)
                    self.Game.check_game_outcome()
                    return True
            ...
            if new_square in black_kingcastle_squares:
                if self.move.is_legal('O-O', "KING"):
                    ...
                    rook.update(self.positionHandler.getPositionFromSquare(F8), F8)
                    piece.update(self.positionHandler.getPositionFromSquare(G8), G8)
                    self.Game.check_game_outcome()
                    return True
            if new_square in black_queencastle_squares:
                if self.move.is_legal('O-O-O', "KING"):
                    ...
                    rook.update(self.positionHandler.getPositionFromSquare(D8), D8)
                    piece.update(self.positionHandler.getPositionFromSquare(C8), C8)
                    self.Game.check_game_outcome()
                    return True
